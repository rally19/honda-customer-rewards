    ...
    public function __invoke(Request $request)
    {
        ...
        $recentClaims = PointExchange::with(['reward:id,name,image_url', 'admin:id,name'])
            ...
            ->map(fn ($exchange) => [
                ...
                'admin_id' => $exchange->admin_id ? (string) $exchange->admin_id : null,
                'admin_name' => $exchange->admin?->name ?? ($exchange->admin_id ? 'Admin #'.$exchange->admin_id : ($exchange->status === 'hold' ? 'Menunggu Review' : 'Staf AHASS')),
        ...
        $recentScans = ActivityHistory::with(['admin:id,name'])
            ...
            ->map(fn ($history) => [
                ...
                'admin_name' => $history->admin?->name ?? ($history->admin_id ? 'Admin #'.$history->admin_id : 'Staf AHASS'),
