...
use App\Models\User;
use App\Services\CustomTwoFactorAuthenticationProvider;
    ...
    public function boot(): void
    {
        ...
        VerifyEmail::toMailUsing(function ($notifiable, $url) {
            return (new MailMessage)
                ->subject('Verifikasi Alamat Email - Honda Customer Rewards')
                ->greeting('Halo, '.$notifiable->name.'!')
                ->line('Terima kasih telah mendaftar di program loyalitas Honda Customer Rewards.')
                ->line('Silakan klik tombol di bawah ini untuk memverifikasi alamat email Anda:')
                ->action('Verifikasi Alamat Email Sekarang', $url)
    ...
    private function configureViews(): void
    {
        ...
        Fortify::twoFactorChallengeView(function (Request $request) {
            ...
            if ($request->session()->has('login.id')) {
                ...
                if ($user) {
                    $emailService = app(TwoFactorEmailService::class);
                    if ($emailService->canResend($user)) {
                        $emailService->sendCode($user);
                    }
                    $parts = explode('@', $user->email);
                    $prefix = $parts[0];
                    $maskedPrefix = strlen($prefix) > 2 ? substr($prefix, 0, 2).str_repeat('*', strlen($prefix) - 2) : $prefix.'***';
                    $maskedEmail = $maskedPrefix.'@'.$parts[1];
                }
            }
